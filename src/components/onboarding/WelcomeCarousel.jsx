...
function WelcomeCarousel({ error, onAlreadyHaveAccount, onGetStarted, saving }) {
  ...
  return (
    <section
      ...
      className="flex min-h-[calc(100dvh-130px)] w-full max-w-[980px] flex-col justify-between overflow-hidden rounded-[28px] border border-white/10 bg-[#0A0C0F] p-5 shadow-glow sm:min-h-[640px] sm:p-8 lg:grid lg:grid-cols-[1fr_0.95fr] lg:items-center lg:gap-10 lg:p-10"
      onTouchEnd={(event) => {
    ...
    >
      <button
        ...
        className="flex min-h-[280px] items-center justify-center rounded-[24px] transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#9CCBFF] sm:min-h-[360px]"
        onClick={next}
        style={{
        ...
        type="button"
      >
        <AtseenEyeMark className="h-36 w-56 animate-[atseen-eye-pulse_3s_ease-in-out_infinite] sm:h-44 sm:w-72" />
      </button>
      <div className="flex flex-1 flex-col justify-end pt-8 lg:justify-center lg:pt-0">
        ...
        <h1 id="welcome-slide-title" className="mt-4 max-w-[560px] text-[clamp(2.25rem,7vw,4.4rem)] font-black leading-[0.96] tracking-normal text-white">{slide.title}</h1>
        <p className="mt-5 max-w-[520px] text-base leading-7 text-white/62 sm:text-lg" aria-live="polite">{slide.description}</p>
        <div className="mt-7 flex gap-2" role="tablist" aria-label="Welcome slides">
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-[0.8fr_1fr]">
          <button className="rounded-2xl border border-white/10 px-5 py-4 text-base font-black text-white/62 transition hover:bg-white/5 hover:text-white disabled:opacity-40" disabled={index === 0 || saving} onClick={previous} type="button">
            Previous
          </button>
          <button className="rounded-2xl bg-[#9CCBFF] px-5 py-4 text-base font-black text-[#0A0C0F] transition hover:bg-[#6FA9E8] disabled:opacity-60" disabled={saving} onClick={next} type="button">
