...
function SeenFeedCard({ item, target }) {
  ...
  const shareMutation = useMutation({ mutationFn: () => complete(engagementQuery.data?.viewerShared ? publicationService.removeSeenShare(item.id) : publicationService.shareSeen(item.id)) });
  ...
  return <article className="seen-feed-card">
    ...
    <div className="seen-feed-body">
      ...
      <div className="seen-feed-actions" aria-label="Seen actions">
        ...
        <button aria-label={engagement.viewerShared ? "Remove Seen from profile" : "Share Seen to profile"} className={engagement.viewerShared ? "text-atseen-blue" : ""} disabled={shareMutation.isPending} onClick={() => shareMutation.mutate()} type="button"><FiShare2 /> <small>{engagement.shareCount}</small></button>
        <Link aria-label="Open Seen" to={target}><FiBookmark /></Link>
      </div>
