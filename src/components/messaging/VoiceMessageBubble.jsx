...
export default function VoiceMessageBubble({ audio, label = "Voice message", mine = false }) {
  ...
  return <div className="flex w-[min(190px,62vw)] min-w-0 max-w-full items-center gap-2.5 sm:w-auto sm:min-w-[190px]">
    ...
    <div className="min-w-0 flex-1">
      ...
      <div className={`mt-0.5 flex justify-between text-[9px] ${mine ? "text-atseen-bg/60" : "text-atseen-muted"}`}><span>{failed ? "Audio unavailable" : playing ? clock(current) : label}</span><span>{clock(duration)}</span></div>
    </div>
  </div>;
}
