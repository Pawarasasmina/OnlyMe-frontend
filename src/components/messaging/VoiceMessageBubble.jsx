...
export default function VoiceMessageBubble({ audio, mine = false }) {
  ...
  return <div className="flex min-w-[190px] items-center gap-2.5">
    ...
    <div className="min-w-0 flex-1">
      <button aria-label={`Seek voice message. Current position ${clock(current)} of ${clock(duration)}`} className="flex h-8 w-full touch-none cursor-ew-resize items-center gap-[2px]" disabled={failed || duration <= 0} onDoubleClick={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === "ArrowLeft") { event.preventDefault(); seek(current - 5); } if (event.key === "ArrowRight") { event.preventDefault(); seek(current + 5); } }} onPointerDown={startWaveformDrag} onPointerMove={dragWaveform} type="button">{waveform.slice(0, 36).map((level, index) => {
