...
import { canAccessSharedSocial, defaultDestinationFor, destinationForUser, isOnboardingComplete, onboardingPathFor, socialCapabilitiesFor } from "./socialAccess.js";
...
test("incomplete fan or creator users are routed through onboarding", () => {
  assert.equal(isOnboardingComplete({ role: "fan", onboarding: { status: "not_started" } }), false);
  assert.equal(destinationForUser({ role: "creator", onboarding: { status: "in_progress", currentStep: "interests" } }), "/onboarding/interests");
  assert.equal(onboardingPathFor({ role: "fan", onboarding: { status: "in_progress", currentStep: "light-your-world" } }), "/onboarding/light-your-world");
  assert.equal(destinationForUser({ role: "fan", onboarding: { status: "completed" } }), "/wall");
  assert.equal(destinationForUser({ role: "admin", onboarding: { status: "not_started" } }), "/admin/dashboard");
});
