...
export const SEEN_VISIBILITIES=["PUBLIC","FRIENDS","LINK_ONLY"];
...
export function seenCompleteness(p){const e=[];if(!p?.title?.trim())e.push("Title is required");if(!p?.summary?.trim())e.push("Summary is required");if(!p?.category?.trim())e.push("Category is required");if(!SEEN_VISIBILITIES.includes(p?.visibility||"PUBLIC"))e.push("Choose a valid Audience");if(!p?.coverMedia?.secureUrl)e.push("A verified cover is required");if(!p?.chapters?.length)e.push("At least one chapter is required");if((p?.chapters?.length||0)>3)e.push("Seen supports at most three chapters");(p?.chapters||[]).forEach((c,i)=>{if(!c.title?.trim())e.push(`Chapter ${i+1} needs a title`);if(chapterTextCount(c)>2000)e.push(`Chapter ${i+1} exceeds 2,000 characters`)});return e}
