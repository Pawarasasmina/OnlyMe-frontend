...
function normalizeOwner(story = {}) {
  const sourceId = story.owner?.id || story.ownerId || story.creatorId || story.id;
...
export const storyService = {
  ...
  createStory: async ({ formData, onUploadProgress, signal }) => {
    if (STORY_API_ENABLED) {
      return axiosInstance.post("/stories", formData, {
        onUploadProgress,
        signal,
      }).then((response) => normalizeStory(response.data?.data || response.data));
    }
  ...
  replyToStory: async (storyId, message) => {
    if (STORY_API_ENABLED) {
      return axiosInstance.post(`/stories/${storyId}/replies`, { message });
    }
