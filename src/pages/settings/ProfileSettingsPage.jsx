...
const settingsTabs = [
  { label: "Profile", section: "profile", settingsPath: "/settings/profile" },
  { label: "Account", section: "account", settingsPath: "/settings/account" },
  { label: "Privacy", section: "privacy", settingsPath: "/settings/privacy" },
  { label: "Notifications", section: "notifications", settingsPath: "/settings/notifications" },
];
...
function CheckboxField({ checked, description, disabled, label, name, onChange }) {
  return (
    <label className="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 transition hover:border-white/20">
      <span>
        <span className="block text-sm font-semibold text-brand-mist/90">{label}</span>
        {description ? <span className="mt-1 block text-xs leading-5 text-brand-mist/50">{description}</span> : null}
      </span>
      <input checked={checked} disabled={disabled} name={name} onChange={onChange} type="checkbox" />
    </label>
  );
...
function InfoTile({ label, value }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      <p className="text-xs font-semibold uppercase tracking-wide text-brand-mist/45">{label}</p>
      <p className="mt-2 text-sm font-semibold text-white">{value}</p>
    </div>
  );
}

function SelectField({ children, description, label, name, onChange, value }) {
  return (
    <label className="block space-y-2">
      <span className="text-sm font-semibold text-brand-mist/85">{label}</span>
      {description ? <span className="block text-xs leading-5 text-brand-mist/50">{description}</span> : null}
      <select
        className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none focus:border-brand-primary"
        name={name}
        onChange={onChange}
...
function ProfileSettingsPage() {
  ...
  const isFanProfileRoute = location.pathname.startsWith("/fan/profile");
  const visibleTabs = useMemo(
    () =>
      settingsTabs.map((tab) => ({
        ...tab,
        to: isFanProfileRoute ? `/fan/profile${tab.section === "profile" ? "" : `?section=${tab.section}`}` : tab.settingsPath,
      })),
    [isFanProfileRoute]
  );
  const activeSection = useMemo(() => {
    if (isFanProfileRoute) {
      const section = new URLSearchParams(location.search).get("section") || "profile";
      return settingsTabs.some((tab) => tab.section === section) ? section : "profile";
    }

    return settingsTabs.find((tab) => location.pathname === tab.settingsPath)?.section || "profile";
  }, [isFanProfileRoute, location.pathname, location.search]);
  const activeTab = visibleTabs.find((tab) => tab.section === activeSection) || visibleTabs[0];
  ...
  const validateForm = () => {
    ...
    if (role === "creator") {
      ...
      if (form.ppmEnabled && (Number(form.ppmPrice) < 10 || Number(form.ppmPrice) > 1000)) {
        errors.ppmPrice = "PPM price must be between 10 and 1,000 Stars.";
      }
    }
  ...
  const uploading =
    avatarMutation.isPending || removeAvatarMutation.isPending || coverMutation.isPending || removeCoverMutation.isPending;
  const showProfileSection = activeSection === "profile";
  const showAccountSection = activeSection === "account";
  const showPrivacySection = activeSection === "privacy";
  const showNotificationsSection = activeSection === "notifications";
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap gap-2">
        {visibleTabs.map((tab) => (
          <Link
            className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
              activeTab.section === tab.section ? "bg-brand-primary text-white" : "bg-white/10 text-brand-mist/75 hover:bg-white/15"
            }`}
            key={tab.section}
            to={tab.to}
          >
      ...
      {showProfileSection && role !== "admin" ? <ProfileCompletionCard completion={profileData.completion} /> : null}
      ...
      <form className="space-y-6" onSubmit={submit}>
        ...
        {showProfileSection && role === "creator" ? (
          <section className="space-y-5 rounded-3xl border border-white/10 bg-brand-dark/60 p-5">
            <div>
              <h2 className="text-xl font-semibold">Creator settings</h2>
              <p className="mt-1 text-sm text-brand-mist/60">Subscription, discovery, and messaging settings.</p>
        ...
        {showPrivacySection ? (
          <section className="space-y-5 rounded-3xl border border-white/10 bg-brand-dark/60 p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-brand-mist/60">Control what other people can see and how your account appears across Atseen.</p>
            </div>
            <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
              {role !== "admin" ? (
                <SelectField
                  description="Public profiles can be viewed by username. Private profiles keep your fan details hidden from public pages."
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <InfoTile label="Email privacy" value="Your email is never shown on public profiles." />
              <InfoTile label="Payment privacy" value="Wallet and payment details are never exposed here." />
              <InfoTile label="Account safety" value="Internal IDs, tokens, and moderation notes stay hidden." />
            </div>
          </section>
        ) : null}
        ...
        {showNotificationsSection ? (
          <section className="space-y-5 rounded-3xl border border-white/10 bg-brand-dark/60 p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-brand-mist/60">Choose where Atseen can send account, creator, and product updates.</p>
            </div>
            <div className="grid gap-3 md:grid-cols-3">
              <CheckboxField
                checked={form.notificationPreferences.email}
                description="Receive important account and creator updates by email."
              ...
              <CheckboxField
                ...
                description="Show alerts inside your Atseen dashboard."
              ...
              {role === "admin" ? (
                <CheckboxField
                  checked={form.notificationPreferences.security}
                  description="Receive security and operational alerts."
                ...
                <CheckboxField
                  checked={form.notificationPreferences.marketing}
                  description="Receive product news, launch notes, and recommendations."
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <InfoTile label="Creator updates" value="Uses your email and in-app notification settings." />
              <InfoTile label="Wallet and subscription alerts" value="Critical account notices may still be shown for safety." />
            </div>
          </section>
        ) : null}
