...
const BackLink = () => <Link className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-atseen-muted transition hover:text-atseen-blue" to="/profile"><FiArrowLeft /> Back to Profile</Link>;
const EmptySection = ({ children }) => <div className="rounded-2xl border border-dashed border-atseen-line p-8 text-center text-sm text-atseen-muted">{children}</div>;

export default function SavedPage() {
  const query = useQuery({ queryKey: ["saved-content"], queryFn: () => savedService.list().then((response) => response.data.data), retry: false });
  if (query.isLoading) return <div><BackLink /><h1 className="text-3xl font-black">Saved</h1><LoadingSkeleton className="mt-5 h-48" count={2} /></div>;
  if (query.isError) return <div><BackLink /><h1 className="text-3xl font-black">Saved</h1><div className="mt-5 rounded-2xl border border-atseen-line p-8 text-center"><p className="text-atseen-muted">Unable to load saved posts.</p><button className="mt-3 font-bold text-atseen-blue" onClick={() => query.refetch()} type="button">Try again</button></div></div>;
  const seens = query.data?.seens || [];
  const wallPosts = query.data?.wallPosts || [];
  return <div><BackLink /><header><p className="text-[10px] font-bold uppercase tracking-[.2em] text-atseen-muted">Your private collection</p><div className="mt-1 flex items-center gap-3"><FiBookmark className="text-2xl text-atseen-blue" /><h1 className="text-3xl font-black">Saved</h1></div><p className="mt-2 text-sm text-atseen-muted">Only you can see the Seens and Wall posts saved here.</p></header>
