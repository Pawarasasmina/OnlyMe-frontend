...
import { FiArrowLeft, FiCalendar, FiPlusCircle } from "react-icons/fi";
...
function UnifiedProfilePage({ owner = false }) {
  ...
  const body = (() => {
    ...
    if (!owner && profile.role === "creator") {
      return <PublicProfileExperience data={data} />;
    }
    ...
    return <>
      ...
      {viewerCapabilities.isOwner && canCreateStory(user) ? (
        <button
          className="mt-3 inline-flex items-center gap-2 rounded-2xl border border-atseen-blue/45 bg-atseen-blue/10 px-4 py-3 text-sm font-bold text-atseen-blue"
  ...
  if (profileQuery.data?.profile?.role === "creator") return body;
