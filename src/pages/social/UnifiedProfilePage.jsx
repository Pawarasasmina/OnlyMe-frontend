...
function UnifiedProfilePage({ embedded = false, owner = false }) {
  ...
  const body = (() => {
    ...
    return <>
      ...
      {tab === "shared" ? <div className="mt-4"><h2 className="mb-3 text-sm font-bold">Shared Seens</h2><ProfileContentGrid content={data.sharedSeens || []} kind="seens" /><h2 className="mb-3 mt-6 text-sm font-bold">Shared Wall posts</h2><div>{data.sharedWallPosts?.map((post) => <FeedPost key={post.id} post={post} />)}{!data.sharedWallPosts?.length ? <p className="text-sm text-atseen-muted">No shared Wall posts yet.</p> : null}</div></div> : null}
