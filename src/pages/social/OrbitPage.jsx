...
function OrbitCreator({ creator, index }) {
  const planets = creator.planets.filter((planet) => planet.kind === "PREMIUM_WORLD");
  const angle = (index * 137.5) * Math.PI / 180;
  const radius = 31 + (index % 3) * 7;
  const style = { left: `${50 + Math.cos(angle) * radius}%`, top: `${48 + Math.sin(angle) * radius}%` };
  return <Link className="orbit-creator-light" style={style} to={`/profile/${creator.username}`}><span className="orbit-creator-avatar">{creator.avatar ? <img alt="" src={resolveMediaUrl(creator.avatar)} /> : creator.name?.slice(0, 1)}</span>{planets.map((planet, planetIndex) => <span className={`orbit-riding-planet planet-${planetIndex}`} key={planet.id}>{planet.emoji}</span>)}<span className="orbit-creator-name">{creator.name}</span></Link>;
}
...
export default function OrbitPage() {
  ...
  const creators = (query.data || []).map((creator) => ({ ...creator, planets: creator.planets.filter((planet) => planet.kind === "PREMIUM_WORLD") }));
