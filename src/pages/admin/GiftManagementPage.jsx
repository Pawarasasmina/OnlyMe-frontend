...
const empty = { name: "", stars: 50, categoryId: "", displayScale: 100, imagePositionX: 0, imagePositionY: 0, isActive: true, image: null };
...
function GiftForm({ categories, gift, onClose, onSaved }) {
  const [form, setForm] = useState(gift ? { ...gift, categoryId: gift.category?.id || "", imagePositionX: gift.imagePositionX || 0, imagePositionY: gift.imagePositionY || 0, image: null } : { ...empty, categoryId: categories[0]?.id || "" });
  ...
  const submit = async (event) => {
    ...
    ["name", "stars", "categoryId", "displayScale", "imagePositionX", "imagePositionY", "isActive"].forEach((key) => data.append(key, form[key]));
    ...
    <form className="max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-7" onSubmit={submit}>
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-[1fr_15rem]">
        <div className="space-y-4">
          ...
          <label className="block text-sm font-semibold">Category<select className={fieldClass} onChange={(e) => set("categoryId", e.target.value)} required value={form.categoryId}><option value="" disabled>Choose a category</option>{categories.filter((category) => category.isActive || category.id === form.categoryId).map((category) => <option key={category.id} value={category.id}>{category.name}{category.isActive ? "" : " (hidden)"}</option>)}</select></label>
          ...
          <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-200 p-4 text-sm font-bold text-slate-600 hover:border-orange-300"><FiUploadCloud />{form.image ? form.image.name : gift ? "Replace image" : "Choose PNG, animated WebP, or GIF"}<input accept="image/png,image/webp,image/gif,image/jpeg" className="hidden" onChange={(e) => set("image", e.target.files?.[0] || null)} type="file" /></label>
          <p className="-mt-2 text-xs leading-5 text-slate-500">Animated WebP and GIF gifts loop automatically in the fan gift popup. Transparent backgrounds work best.</p>
...
function CategoryManager({ categories, onChanged }) {
  const [name, setName] = useState(""), [busy, setBusy] = useState(""), [error, setError] = useState("");
  const create = async (event) => { event.preventDefault(); if (!name.trim()) return; setBusy("new"); setError(""); try { await adminService.createGiftCategory(name.trim()); setName(""); await onChanged(); } catch (e) { setError(e.response?.data?.message || "Category could not be created."); } finally { setBusy(""); } };
  const rename = async (category) => { const next = prompt("Category name", category.name)?.trim(); if (!next || next === category.name) return; setBusy(category.id); setError(""); try { await adminService.updateGiftCategory(category.id, { name: next }); await onChanged(); } catch (e) { setError(e.response?.data?.message || "Category could not be updated."); } finally { setBusy(""); } };
  const toggle = async (category) => { setBusy(category.id); setError(""); try { await adminService.updateGiftCategory(category.id, { isActive: !category.isActive }); await onChanged(); } catch (e) { setError(e.response?.data?.message || "Category could not be updated."); } finally { setBusy(""); } };
  const remove = async (category) => { if (!confirm(`Delete “${category.name}”?`)) return; setBusy(category.id); setError(""); try { await adminService.deleteGiftCategory(category.id); await onChanged(); } catch (e) { setError(e.response?.data?.message || "Category could not be deleted."); } finally { setBusy(""); } };
  return <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div><h2 className="font-black">Gift categories</h2><p className="mt-1 text-sm text-slate-500">Create and manage the sections fans see in the gift picker.</p></div><form className="mt-4 flex gap-2" onSubmit={create}><input className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-orange-400" maxLength={60} onChange={(event) => setName(event.target.value)} placeholder="New category name" value={name} /><button className="rounded-xl bg-slate-900 px-4 text-sm font-bold text-white disabled:opacity-50" disabled={busy === "new"} type="submit"><FiPlus className="inline" /> Add</button></form>{error ? <p className="mt-3 text-sm text-red-600">{error}</p> : null}<div className="mt-4 flex flex-wrap gap-2">{categories.map((category) => <div className={`flex items-center gap-2 rounded-full border px-3 py-2 text-xs ${category.isActive ? "border-slate-200" : "border-slate-200 bg-slate-100 text-slate-400"}`} key={category.id}><b>{category.name}</b><button disabled={busy === category.id} onClick={() => rename(category)} type="button"><FiEdit2 /></button><button className={category.isActive ? "text-emerald-600" : "text-slate-400"} disabled={busy === category.id} onClick={() => toggle(category)} type="button">{category.isActive ? "Visible" : "Hidden"}</button><button className="text-red-500" disabled={busy === category.id} onClick={() => remove(category)} type="button"><FiTrash2 /></button></div>)}</div></section>;
}
...
export default function GiftManagementPage() {
  const [gifts, setGifts] = useState([]), [categories, setCategories] = useState([]), [editing, setEditing] = useState(undefined), [formOpen, setFormOpen] = useState(false), [loading, setLoading] = useState(true), [reordering, setReordering] = useState(false), [error, setError] = useState("");
  const load = async () => { setLoading(true); setError(""); try { const data = (await adminService.getGifts()).data.data; setGifts(data.gifts); setCategories(data.categories || []); } catch (e) { setError(e.response?.data?.message || "Gift catalog could not be loaded."); } finally { setLoading(false); } };
  useEffect(() => { load(); }, []);
  ...
  return <div><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-widest text-orange-500">Dream support</p><h1 className="mt-1 text-3xl font-black">Gift catalog</h1><p className="mt-2 text-sm text-slate-500">Manage the image gifts fans can send from a creator’s Dream.</p></div><button className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-4 py-3 font-bold text-white" onClick={() => open(null)} type="button"><FiPlus />Add gift</button></div>
    ...
    {formOpen ? <GiftForm categories={categories} gift={editing} onClose={() => setFormOpen(false)} onSaved={() => { setFormOpen(false); load(); }} /> : null}
  </div>;
}
