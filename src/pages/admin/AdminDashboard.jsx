...
function AdminDashboard() {
  ...
  const moderationQuery = useQuery({ queryKey: ["admin", "dashboard", "pending-content"], queryFn: () => adminContentService.listModerationContent({ status: "PENDING_REVIEW", page: 1, limit: 5 }).then((response) => response.data.data) });
  if (query.isLoading || moderationQuery.isLoading) return <div className="text-slate-600"><Loader label="Loading admin dashboard..." /></div>;
  if (query.isError || moderationQuery.isError) return <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">Unable to load dashboard. <button className="font-bold underline" onClick={() => { query.refetch(); moderationQuery.refetch(); }} type="button">Retry</button></div>;

  const stats = query.data;
  const pendingContent = moderationQuery.data?.pagination?.total || 0;
  const cards = [
  ...
  return <div className="mx-auto max-w-[1600px]">
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-[1.4fr_0.8fr]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="font-bold">Quick management</h2><p className="mt-1 text-xs text-slate-500">Jump directly to common administration tasks.</p></div></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><Link className="group flex items-center justify-between rounded-xl border border-slate-200 p-4 hover:border-blue-300 hover:bg-blue-50/50" to="/admin/fans"><span><strong className="block text-sm">Manage fans</strong><small className="text-slate-500">{stats.fans} accounts</small></span><FiArrowRight className="text-slate-400 group-hover:text-blue-500" /></Link><Link className="group flex items-center justify-between rounded-xl border border-slate-200 p-4 hover:border-orange-300 hover:bg-orange-50/50" to="/admin/creator-verifications"><span><strong className="block text-sm">Review verifications</strong><small className="text-slate-500">{stats.pendingCreators} currently waiting</small></span><FiArrowRight className="text-slate-400 group-hover:text-orange-500" /></Link><Link className="group flex items-center justify-between rounded-xl border border-slate-200 p-4 hover:border-violet-300 hover:bg-violet-50/50" to="/admin/content-moderation"><span><strong className="block text-sm">Review content</strong><small className="text-slate-500">{pendingContent} requiring review</small></span><FiArrowRight className="text-slate-400 group-hover:text-violet-500" /></Link></div><div className="mt-4 border-t border-slate-100 pt-4"><p className="text-xs font-bold uppercase text-slate-400">Recently submitted</p><div className="mt-2 space-y-2">{(moderationQuery.data?.items || []).slice(0, 3).map((item) => <Link className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm" key={item.id} to={`/admin/content-moderation/${item.id}`}><span className="truncate font-semibold">{item.title}</span><span className="text-xs text-slate-400">@{item.creator?.username}</span></Link>)}{!moderationQuery.data?.items?.length ? <p className="text-sm text-slate-500">No content currently requires review.</p> : null}</div></div></section>
