...
import { FiArrowLeft, FiFilm, FiPlus } from "react-icons/fi";
...
function SeenManagerCover({ media }) {
  const url = media?.secureUrl || media?.url || "";

  if (!url) {
    return (
      <div className="grid h-full w-full place-items-center bg-atseen-surface-2 text-sm font-black text-atseen-muted">
        @seen
      </div>
    );
  }
  ...
  if (media.mediaType === "VIDEO" || media.resourceType === "video") {
    return (
      <div className="relative h-full w-full">
        ...
        <span className="absolute bottom-2 right-2 grid h-7 w-7 place-items-center rounded-full bg-black/65 text-white backdrop-blur">
          <FiFilm />
        </span>
      </div>
    );
  ...
  return <img alt="" className="h-full w-full object-cover" src={url} />;
}

function SeenManagerCard({ item }) {
  const target = item.id ? `/studio/seens/${item.id}` : "/studio/seens";

  return (
    <Link className="group overflow-hidden rounded-2xl border border-atseen-line bg-atseen-surface transition hover:border-atseen-blue/45" to={target}>
      <div className="aspect-square overflow-hidden bg-atseen-surface-2">
        <SeenManagerCover media={item.coverMedia} />
      </div>
      <div className="p-3">
        <strong className="block truncate text-sm text-white">{item.title}</strong>
        <p className="mt-1 text-xs text-atseen-muted">{item.chapterCount} {item.chapterCount === 1 ? "chapter" : "chapters"}</p>
      </div>
    </Link>
  );
}
